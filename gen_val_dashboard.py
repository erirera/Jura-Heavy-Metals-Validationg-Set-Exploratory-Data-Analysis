...
<div class="card" style="margin-bottom:36px">
  <div class="card-title">Sample Locations &mdash; Color-coded by concentration</div>
  <div class="controls">Metal: <select id="spatialSel"></select></div>
  <canvas id="spatialCanvas" style="max-height:400px"></canvas>
</div>
...
// Spatial Map
var spatialSel=document.getElementById('spatialSel');
METALS.forEach(function(m){spatialSel.appendChild(new Option(m,m));});
var spatialChart=null;
function buildSpatial(){
  var m=spatialSel.value;
  var vals=ROWS.map(function(r){return r[m];});
  var mn=Math.min.apply(null,vals), mx=Math.max.apply(null,vals);
  var pts=ROWS.map(function(r){return {x:r.x,y:r.y};});
  var bgs=ROWS.map(function(r){return metalColor(r[m],mn,mx);});
  if(spatialChart) spatialChart.destroy();
  spatialChart=new Chart(document.getElementById('spatialCanvas'),{
    type:'scatter',
    data:{datasets:[{label:m,data:pts,backgroundColor:bgs,pointRadius:7,pointHoverRadius:9}]},
    options:{plugins:{legend:{display:false},
      tooltip:{callbacks:{label:function(ctx){
        var r=ROWS[ctx.dataIndex];
        var rk=['Argovian','Kimmeridgian','Sequanian','Portlandian','Quaternary'][r.rock-1];
        var lk=['Forest','Pasture','Meadow','Tillage'][r.land-1];
        return ['Loc '+r.id, m+': '+r[m]+' ppm','Rock: '+rk,'Land: '+lk];
      }}}},
      scales:{
        x:{title:{display:true,text:'X (km)',color:'#8888bb'},ticks:{color:'#6666aa'},grid:{color:'#1e1e40'}},
        y:{title:{display:true,text:'Y (km)',color:'#8888bb'},ticks:{color:'#6666aa'},grid:{color:'#1e1e40'}}
      }}
  });
}
spatialSel.addEventListener('change',buildSpatial);
buildSpatial();
...
out = os.path.join(BASE, "jura_validation_dashboard.html")
